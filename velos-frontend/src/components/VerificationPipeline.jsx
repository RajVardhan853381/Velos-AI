...
import { Upload, CheckCircle, Loader2, AlertCircle, FileText, Sparkles, Award, Shield, Eye, Target, TrendingUp, Clock, User, Mail, MapPin, Briefcase, Code } from 'lucide-react';
  ...
  const loadSampleData = async () => {
    try {
      const [resumeRes, jobRes] = await Promise.all([
        fetch('http://localhost:8000/api/sample-resume'),
        fetch('http://localhost:8000/api/sample-job-description')
      ]);
      
      if (resumeRes.ok && jobRes.ok) {
        const resumeData = await resumeRes.json();
        const jobData = await jobRes.json();
        setResumeText(resumeData.resume || '');
          ...
          <div className="space-y-6">
            ...
            <div className="flex gap-4">
              ...
              <motion.button
                ...
                disabled={loading}
                className="flex-1 bg-white/80 hover:bg-white backdrop-blur-sm border border-emerald-200 text-slate-700 px-8 py-4 rounded-xl font-medium shadow-sm transition-all active:scale-95 disabled:opacity-50"
              >
                Demo Mode
              </motion.button>
            </div>
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-3 mt-3">
                    ...
                    <div className="text-sm">
                      ...
                      <p className="text-lg font-bold text-gray-800">{result.verified_skills?.length || 5}</p>
                    </div>
                  </div>
                </motion.div>
