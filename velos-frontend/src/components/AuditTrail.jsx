...
const AuditTrail = () => {
  ...
  const exportLogs = () => {
    const csv = [
      ['Time', 'User', 'Action', 'Module', 'Status'],
      ...filteredLogs.map(log => [log.time, log.user, log.action, log.module, log.status])
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      >
        {[
          ...
          { label: 'Success Rate', value: `${Math.round((auditLogs.filter(l => l.status === 'success' || l.status === 'passed').length / auditLogs.length) * 100)}%`, color: 'from-emerald-400 to-green-400' },
