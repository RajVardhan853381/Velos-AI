  ...
  const fetchGodModeData = async () => {
    try {
      const [insightsRes, agentsRes, healthRes] = await Promise.all([
        ...
        fetch('http://localhost:8000/api/agents').catch(() => null),
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-violet-950 via-purple-900 to-indigo-950 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-violet-300 text-lg">Initializing God Mode...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-white p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <MetricCard icon={Clock} label="Avg Processing" value={`${insights?.avg_processing_time?.toFixed(1) || 0}s`} status={insights?.avg_processing_time < 3 ? 'good' : 'warning'} gradient="from-cyan-500 to-blue-500" />
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white/95 backdrop-blur-sm border border-violet-100 shadow-xl rounded-2xl p-6 mb-8 relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">
          ...
          <div className="bg-slate-50 rounded-xl p-4 border border-slate-200">
            ...
            <p className="text-2xl font-bold text-slate-900">{health?.memory_usage ? `${health.memory_usage.toFixed(1)}%` : '0%'}</p>
          </div>
        </div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} className="mt-8 text-center">
        <div className="inline-flex items-center gap-2 bg-white/95 backdrop-blur-sm border border-violet-200 shadow-md rounded-full px-6 py-3">
          ...
          <p className="text-slate-600 text-sm">Live monitoring • Mock data enabled</p>
        </div>
      </motion.div>
    </div>
  );
