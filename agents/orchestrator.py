...
class VelosOrchestrator:
    ...
    def run_verification_pipeline(self, raw_resume: str,
        ...
        result["agent_1_status"] = agent1_result["status"]
        result["agent_1_reason"] = agent1_result["reason"]
        result["years_exp"] = agent1_result["years_exp"]
        ...
        if self.diff_engine and agent1_result.get("redacted_text"):
            ...
            print(f"   ✓ {diff_report['stats']['deletions']} PII items removed")
            print(f"   ✓ {diff_report['stats']['insertions']} placeholders added")
            print(f"   ✓ Redaction rate: {diff_report['stats']['redaction_rate']}%")
        else:
        ...
        if agent1_result["status"] == "FAIL":
            ...
            result["final_reason"] = agent1_result["reason"]
            
            self.audit_db.update_candidate(candidate_id, {
                "status": "REJECTED",
                "years_exp": agent1_result["years_exp"],
        ...
        print(f"✅ Agent 1 PASSED: {agent1_result['reason']}")
        
        # Issue ELIGIBILITY credential via Zynd Protocol
        if ZYND_AVAILABLE and self.protocol and candidate_did and self.agent1_identity:
            candidate_did_str = self._get_did_string(candidate_did)
            eligibility_credential = self.protocol.issue_credential(
                issuer_did=self.agent1_identity["id"],
            ...
            if self.agent2_identity:
                ...
                agent1_msg = self.protocol.send_message(
                    sender_did=self.agent1_identity["id"],
        ...
        agent2_result = self.validator.process(
            agent1_result["clean_data"],
            job_description,
            agent1_result["clean_token"],
            vector_store=self.vector_store,
            candidate_id=candidate_id
        )
        result["pipeline_stages"]["agent_2"] = agent2_result
        result["agent_2_status"] = agent2_result["status"]
        result["agent_2_score"] = agent2_result["score"]
        result["agent_2_reason"] = agent2_result["reason"]
        ...
        if agent2_result["status"] == "FAIL":
            ...
            result["final_reason"] = agent2_result["reason"]
            
            self.audit_db.update_candidate(candidate_id, {
                "status": "REJECTED",
                "years_exp": agent1_result["years_exp"],
                "match_score": agent2_result["score"],
        ...
        print(f"✅ Agent 2 PASSED: {agent2_result['reason']}")
        
        # Issue SKILL credential via Zynd Protocol
        if ZYND_AVAILABLE and self.protocol and candidate_did and self.agent2_identity and self.agent3_identity:
            skill_credential = self.protocol.issue_credential(
                issuer_did=self.agent2_identity["id"],
                subject_did=candidate_did.get("id", ""),
            ...
            agent2_msg = self.protocol.send_message(
                sender_did=self.agent2_identity["id"],
                recipient_did=self.agent3_identity["id"],
                content=json.dumps({
                    "candidate_did": candidate_did.get("id", ""),
        ...
        result["verification_questions"] = agent3_questions["questions"]
        
        # Save audit entries
        for entry in agent3_questions.get("audit_log", []):
            self.audit_db.save_audit_entry(candidate_id, entry)
        
        print(f"✅ Agent 3 generated {len(agent3_questions['questions'])} questions")
    ...
    def evaluate_candidate_answers(self, qa_pairs: List[Dict],
        ...
        self.current_result["agent_3_status"] = agent3_eval["status"]
        self.current_result["agent_3_authenticity"] = agent3_eval["authenticity_score"]
        self.current_result["agent_3_reason"] = agent3_eval["reason"]
        ...
        if agent3_eval["status"] == "PASS":
            ...
            self.current_result["final_reason"] = agent3_eval["reason"]
            ...
            print(f"❌ FINAL DECISION: REJECTED - {agent3_eval['reason']}")
        ...
        if self.ledger_manager:
            ...
            hashable_result = {
                ...
                "agent_3_status": agent3_eval["status"],
                "agent_3_authenticity": agent3_eval["authenticity_score"],
        ...
        return {
            "candidate_id": cid,
            "agent_3_status": agent3_eval["status"],
            "agent_3_reason": agent3_eval["reason"],
            "agent_3_authenticity": agent3_eval["authenticity_score"],
    ...
    def get_agent_trust_scores(self) -> Dict[str, float]:
        ...
        if ZYND_AVAILABLE and self.protocol:
            return {
                "BlindGatekeeper": self.protocol.get_agent_trust_score(self.agent1_identity["id"]) if self.agent1_identity else 1.0,
                "SkillValidator": self.protocol.get_agent_trust_score(self.agent2_identity["id"]) if self.agent2_identity else 1.0,
                "Inquisitor": self.protocol.get_agent_trust_score(self.agent3_identity["id"]) if self.agent3_identity else 1.0
            }
